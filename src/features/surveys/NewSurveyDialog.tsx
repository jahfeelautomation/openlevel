...
import type { NewSurvey } from '../../lib/api'
...
/** Create a survey: name it (slug auto-derives, still editable). A friendly
 *  two-step starter is seeded server-side, so the operator edits a working
 *  multi-step survey from the first moment. */
export function NewSurveyDialog({
  onCancel,
  onCreate,
}: {
  onCancel: () => void
  onCreate: (input: NewSurvey) => Promise<void>
}) {
    ...
    >
      ...
      >
        <div className="border-b border-slate-100 px-5 py-4">
          <h2 className="text-base font-semibold text-slate-900">New survey</h2>
        </div>

        <div className="space-y-4 px-5 py-4">
          <div>
            <Label htmlFor="sv-new-name">Survey name</Label>
            <Input
              id="sv-new-name"
              value={name}
          ...
          <div>
            <Label htmlFor="sv-new-slug">URL slug</Label>
            <div className="flex items-center rounded-lg border border-slate-200 bg-slate-50 px-3 shadow-sm focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/30">
              <span className="text-sm text-slate-400">/surveys/</span>
              <input
                id="sv-new-slug"
                value={effectiveSlug}
                onChange={(e) => {
