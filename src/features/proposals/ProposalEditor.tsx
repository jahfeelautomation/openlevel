...
import type { Contact, ProposalItem, Product } from '../../lib/api'
import { formatMoneyExact, formatPhone } from '../../lib/utils'
import { CatalogPicker } from '../payments/CatalogPicker'
import { proposalTotalCents } from './proposals-meta'
...
const blankItem = (): ProposalItem => ({ description: '', quantity: 1, unit_amount: 0 })

/** Right-pane editor for one draft proposal. Owns no state — it reflects `draft`
 *  and reports every edit through `onChange`, so the center document re-renders
 *  live (and the derived total updates) as the operator types. Only drafts are
 *  editable: once a proposal is sent the recipient may already be reading it, so
...
export function ProposalEditor({
  ...
  const setItems = (next: ProposalItem[]) => onChange({ ...draft, items: next })
  const patchItem = (i: number, patch: Partial<ProposalItem>) =>
    setItems(items.map((x, idx) => (idx === i ? { ...x, ...patch } : x)))
  // Drop a saved catalog product onto the proposal as a new line (price in cents
  ...
  return (
    <div className="flex h-full w-full flex-col border-t border-slate-200 bg-white lg:w-80 lg:shrink-0 lg:border-l lg:border-t-0">
      ...
      <div className="ol-scroll min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
        ...
        <div>
          <Label htmlFor="pr-contact">For</Label>
          <select
            id="pr-contact"
        ...
        <div>
          <Label htmlFor="pr-intro">Intro</Label>
          <Textarea
            id="pr-intro"
            rows={3}
            value={draft.intro}
            onChange={(e) => onChange({ ...draft, intro: e.target.value })}
            placeholder="A line or two on what you're proposing and why."
          />
        </div>
          ...
          <div className="space-y-2.5">
            {items.length === 0 && (
              <p className="rounded-lg border border-dashed border-slate-200 px-3 py-3 text-center text-xs text-slate-400">
                No items yet — add one to quote for it.
              </p>
            )}
            {items.map((it, i) => (
              <div
          ...
          {items.length > 0 && (
            <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
              ...
              <span className="text-sm font-bold tabular-nums text-slate-900">
                {formatMoneyExact(proposalTotalCents(items))}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="border-t border-slate-100 pt-4">
          <Label htmlFor="pr-terms">Terms</Label>
          <Textarea
            id="pr-terms"
            rows={3}
            value={draft.terms}
            onChange={(e) => onChange({ ...draft, terms: e.target.value })}
            placeholder="Validity window, payment schedule, cancellation, etc."
          />
        </div>
      </div>
    </div>
